...
# Create a camera
camera1 = Camera(camR, camTheta, camPhi, camFocalLength, camSensorShape, camSensorSize)

# Create another camera
camera2 = Camera(camR, camTheta, camPhi, camFocalLength, (1600, 2400), camSensorSize)
...
suffix = 0
for rotation in np.arange(0.01, 0.99, 0.05):
    suffix += 1
    universe.spin = rotation
    texturedImage = camera2.shoot(diskPath=disk, spherePath=sphere)
    texturedImage[0].save("pruebita" + "%03d" % suffix + ".png")
