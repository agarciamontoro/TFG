...
\begin{definition}[Variation {\cite[Chap. 9, Def. 2.1]{docarmo79}}]
...
\begin{proposition}
	...
	\[
		\frac{1}{2} E'_f(0) = - \int_0^1 g\left( V(t), \frac{D}{dt}(\frac{dc}{dt}) \right)dt - \sum_{i=1}^k g \left( V(t_i), \frac{dc}{dt}(t_i^+) - \frac{dc}{dt}(t_i^-) \right),
	\]
	where $V$ is the variational field of $f$ and
	\[
		\frac{dc}{dt}(t_i^+) = \lim_{\substack{t \to t_i \\ t > t_i}}, \quad \frac{dc}{dt}(t_i^-) = \lim_{\substack{t \to t_i \\ t < t_i}}
	\]
\end{proposition}

The characterization of geodesics we are looking for comes as a result from \autoref{pro:energyprop}.
