...
\begin{definition}[Ricci tensor]
	The Ricci tensor $\operatorname{Ric}$ of a semi-Riemannian metric $g$ is defined as the following contraction of the Riemann curvature tensor $R$ of $g$:
	\[
		\operatorname{Ric}(X,Y) \defeq \operatorname{trace}(V \mapsto R(V, X)Y ),
	\]
	that is, $\operatorname{Ric}(X,Y) =  \sum_i \omega^i (R(X_i, X)Y)$; where $\{X_i\}$ is a local basis of vector fields and $\{\omega^i\}$ is its dual basis of one-forms.
\end{definition}

THerefore, for a coordinate system $(x_1, \dots, x_n)$ we have
\[
	\operatorname{Ric} = \sum_{j,k} \operatorname{Ric}\left( \pd{}{x^j},\pd{}{x^j}  \right) dx^j \otimes dx^k,
\]
where the components $\operatorname{Ric}(\pd{}{x^j}, \pd{}{x^k})$ of $\operatorname{Ric}$ are given by
\[
	\operatorname{Ric}(\pd{}{x^j}, \pd{}{x^k}) = \sum_i dx^i \left( R(\pd{}{x^i}, \pd{}{x^j})\pd{}{x^k} \right) = \sum_i R^i_{ijk}.
\]

Equivalently,
\[
	\operatorname{Ric}(X,Y)  = \sum_{i,h} g^{ih} g(R(\pd{}{x^i}, X)Y, \pd{}{x^h}).
\]

\begin{definition}
	The scalar curvature $S$ of $g$ is the contraction of the $(1,1)$ tensor field $g$-equivalent to the Ricci tensor. In terms of coordinates:
	\[
		S = \sum_{i,j} g^{ij}(\operatornname{Ric})_{ij} = \sum_{i,j,k} g^{ij} R^k_{kij}
	\]
\end{definition}

From the second identity of Bianchi we get \cite[p. 88]{oneill83}
\[
	\nabla S = 2 \operatorname{div} \hat{\operatorname{Ric}},
\]
where $\nabla S$ is the gradient of te scalar curvature, $\hat{\operatorname{Ric}}$ is the contravariant tensor field $g$-equivalent to $\operatorname{Ric}$ and $\operatorname{div}$ is the divergence acting on symmetric $(2,0)$ tensor fields.
