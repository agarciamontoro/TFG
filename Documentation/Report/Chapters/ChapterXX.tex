...
The idea of the manifold will be understood, then, taking pieces of the manifold and lowering\unsure{Mmm... not sure about the vern.} them to the real word; \ie, the Euclidean space, where we will be able to \emph{physically} touch the manifold.
...
As the differentiability is a local concept, the property that the manifolds are locally Euclidean is enough to generalize it.

\begin{definition}
	Let $F \colon M \to N$ a map between two differentiable manifolds: $M$ and $N$. F is said to be \emph{differentiable} or \emph{smooth} if the following conditions are satisfied:
	\begin{enumerate}
