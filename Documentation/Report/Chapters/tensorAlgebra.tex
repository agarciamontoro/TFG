...
This addition to the naked vector space we have had until now will let us to find an isomorphism between $V$ and its dual, $V^*$ without using bases; however, this is not a \emph{natural} isomorphism as the one seen on \autoref{eq:naturaliso}, as long as we need for the existence of the metric to build it.
...
\begin{proposition}
	The application $g^*$ that maps every $\varphi, \psi \in V^*$ on $g^*(\varphi,\psi) = g(\varphi^\sharp, \psi^\sharp)$ is a metric on $V^*$. Furthermore, $g^*$ is the only metric that allows $\flat$ and $\sharp$ to be isometries\change{Esto no parece que esté muy bien}.
\end{proposition}
...
\begin{proof}
	...
	If $g'$ is another metric that allows $\flat$ to be an isometry\change{Esto no parece que esté muy bien}, then \[g'(v^\flat, w^\flat) = g(v, w).\] But using \autoref{proof:isometry} we see that $g^*(v^\flat, w^\flat) = g'(v^\flat, w^\flat)$. Given that $\flat$ is one\nobreakdash-to\nobreakdash-one we conclude that $g' = g^*$.
\end{proof}
...
\begin{proposition}
	...
	\[
		...
		\quad \textrm{and} \quad
		T'' = \sum_{i,j}^{n} t''_{ij} \varphi^i \otimes \varphi^j,
	\]
	the expressions that relate the tensors are given as
	\[
		t'^i_j = \sum_{k=1}^n g^*(\varphi^k, \varphi^i) t_{kj}
		\quad \textrm{and} \quad
		t''_{ij} = \sum_{k,l = 1}^n g^*(\varphi^k, \varphi^i) g^*(\varphi^l, \varphi^j) t_{kl}.
	\]
\end{proposition}
...
\begin{proof}
	...
	\begin{align*}
		t''_{ij} &= T''(\varphi^i, \varphi^j) = T((\varphi^i)^\sharp, (\varphi^j)^\sharp) = \\
...
\begin{remark}
	...
	\begin{align*}
		t''_{ij} &= \sum_{k,l = 1}^n g^{ki} g^{lj} t_{kl} \\
		\sum_{i,j=1}^n g_{im} g_{jp} t''_{ij} &= \sum_{i,j,k,l = 1}^n \left( g_{im} g^{ki} \right) \left( g_{jp} g^{lj} \right) t_{kl} \\
		\sum_{i,j=1}^n g_{im} g_{jp} t''_{ij} &= \sum_{k,l = 1}^n \delta_m^k \delta_p^l t_{kl} \\
		\sum_{i,j=1}^n g_{im} g_{jp} t''_{ij} &= t_{mp} \\
	\end{align*}

	Fixing the indices to look nicer, we have the final expressions for both changes:
	\[
		t_{ij} = \sum_{k=1}^n g_{ki} t'^k_j
		\quad \textrm{and} \quad
		t_{ij} = \sum_{k,l=1}^n g_{ki} g_{lj} t''_{kl}
	\]
\end{remark}

\autoref{pro:musicalcoord} gives us a great tool to identify tensors of the same order ---although we have seen the proposition for tensors of order 2, it is easily generalized to any other order---; in fact, the operation of multiplying by the metric and obtaining the same tensor with a different type, as shown in \autoref{rem:raiselower}, is known as \emph{raising and lowering indices}. This operation, whose background is in  but what happens when we work with orthonormal basis?

Let us consider again the example of tensors of order 2, but now with $B$ an orthonormal basis; we know from previous discussions that, in this case, $B^*$ is also orthonormal; \ie, the metric on the elements of the basis is
\[
	g^*(\varphi^i, \varphi^j) = \delta_{ij}.
\]

Then, the expressions for the change of coordinates, following the notation of \autoref{pro:musicalcoord}, are even easier:
\[
	t'^i_j = t_{ij} = t''_{ij}
\]