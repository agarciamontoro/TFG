...
\begin{definition}[Tensor product]
	...
	\begin{align*}
		...
		&T(\varphi^1, \dots, \varphi^r, v_1, \dots, v_s) T(\varphi^{r+1}, \dots, \varphi^{r+r'}, v_{s+1}, \dots, v_{s+s'}) \in K,
	\end{align*}
...
\begin{proof}
	...
	To prove that $\mathcal{B}_T$ is linearly independent, it is trivial to see that a linear combination of the elements in $\mathcal{B}_T$ equals the null tensor $T_0$ if and only if all the coefficients are zero.
\end{proof}
...
The notion of tensor help us to understand this duality, as we can see both elements and both operations as the \emph{same} mathematical entity, expressed in different bases with the corresponding change of coordinates.
