...
\begin{proposition}
	Let $B$, $B'$ and $B^*$, $B'^*$ the bases of $V$ and $V^*$ described above.
	
	If we consider two ordered bases obtained from
	\begin{align*}
		\mathcal{B}_T = \{\varphi^{i_1} \otimes \dots \otimes \varphi^{i_r} \otimes v_{j_1} \otimes \dots \otimes v_{j_s} \textrm{ , } &\textrm{where every index moves} \\
		&\textrm{independently from $1$ to $n$}  \}
	\end{align*}
	and
	\begin{align*}
		\mathcal{B}'_T = \{\varphi'^{i_1} \otimes \dots \otimes \varphi'^{i_r} \otimes v'_{j_1} \otimes \dots \otimes v'_{j_s} \textrm{ , } &\textrm{where every index moves} \\
		&\textrm{independently from $1$ to $n$}  \},
	\end{align*}
	the analytic expression of the change of coordinates of a tensor $T \in \tensors_{r,s}(V)$ is the following:
	\begin{align*}
		t'^{k_1,\dots,k_r}_{l_1,\dots,l_s} = \sum b_{i_1}^{k_1} \cdots b_{i_k}^{k_r} a_{l_1}^{j_1} \cdots a_{l_s}^{j_s} t_{j_1,\dots,j_s}^{i_1,\dots,i_r}
	\end{align*}
...
\begin{proof}
	...
	\begin{align*}
		t'^{k_1,\dots,k_r}_{l_1,\dots,l_s} &= T(\varphi'^{k_1}, \dots, \varphi'^{k_r}, v_{l_1}', \dots, v_{l_s}')  = \\
		&= T(\sum_{i_1}b_{i_1}^{k_1}\varphi^{i_1}, \dots, \sum_{i_r}b_{i_r}^{k_r}\varphi{i_r}, \sum_{j_1}^n a_{l_1}^{j_1} v_{j_1}, \dots, \sum_{j_s}^n a_{l_s}^{j_s} v_{j_s}) = \\
...
\[
	t_k^l = \sum_{i,j} a_j^l b_k^i t'^j_i \quad \forall k,l \in \{1,2,\dots,n\}.
\]

This expression can be written using matrix notation as follows:
\begin{equation}
	\label{eq:tensors11}
	(t_k^l) = \mathbf{A} (t'^j_i) \mathbf{A}^{-1}
\end{equation}
...
This is not surprising, as both spaces are isomorphic, and there exists a \emph{natural} isomorphism between them:
\begin{theorem}
	The map
	\begin{align*}
