...
\]
that maps every pair $(\psi, w)$ to the scalar $\psi(v)\varphi(w)$.
...
Let $B = (v_1, \dots, v_n)$ and $B' = (v_1', \dots, v_n')$ be two bases of $V(K)$, and suppose that\change{Exchange a and b roles}
\begin{equation}
	\label{eq:changeV}
	v_j' = \sum_{i=1}^n a_j^i v_i \textrm{, where } a_j^i \in K.
\end{equation}
...
\[
	\varphi^i = \sum_{j=1}^n a_j^i \varphi'^j.
\]

Furthermore, we can see that\change{Exchange a and b roles}
\begin{equation}
	\label{eq:changeV*}
	\varphi'^i = \sum_{l=1}^n b_l^i \varphi'^l,
\end{equation}
where $b_k^i$ is the element placed on the $i$-th row, $l$-th column of  the inverse matrix of $(a^i_j)$.

This change of bases in $V$ a nd $V^*$  is key to understand the following Proposition, which tells us how  the coordinates of a tensor change when the basis of $\tensors_{r,s}(V)$.
...
\begin{proposition}
	...
	Then, the analytic expression of the change of coordinates of a tensor $T \in \tensors_{r,s}(V)$ is the following:\change{Exchange a and b roles}
	\begin{align*}
		t'^{k_1,\dots,k_r}_{\phantom{'}l_1,\dots,l_s} = \sum b_{i_1}^{k_1} \cdots b_{i_k}^{k_r} a_{l_1}^{j_1} \cdots a_{l_s}^{j_s} t_{j_1,\dots,j_s}^{i_1,\dots,i_r}
	\end{align*}
...
\begin{proof}
	...
	\begin{align*}
		...
		&= T(\sum_{i_1}b_{i_1}^{k_1}\varphi^{i_1}, \dots, \sum_{i_r}b_{i_r}^{k_r}\varphi{i_r}, \sum_{j_1} a_{l_1}^{j_1} v_{j_1}, \dots, \sum_{j_s} a_{l_s}^{j_s} v_{j_s}) = \\
		&= \sum b_{i_1}^{k_1} \cdots b_{i_k}^{k_r} a_{l_1}^{j_1} \cdots a_{l_s}^{j_s} T(\varphi^{k_1}, \dots, \varphi^{k_r}, v_{l_1}, \dots, v_{l_s}) = \\
		&= \sum b_{i_1}^{k_1} \cdots b_{i_k}^{k_r} a_{l_1}^{j_1} \cdots a_{l_s}^{j_s} t_{j_1,\dots,j_s}^{i_1,\dots,i_r}
	\end{align*}
\end{proof}

We can study now the particular examples of tensors of order 2.

For $r = s = 1$, the expression of the change of coordinates is\change{Exchange a and b roles}
\[
	t'^k_{\phantom{'}l} = \sum_{i,j}^n b^k_i a_l^j  t^i_j \quad \forall k,l \in \{1,2,\dots,n\}.
\]

This expression can be written using matrix notation as follows:
\begin{equation}
	\label{eq:tensors11}
	(t'^k_{\phantom{'}l}) = \mathbf{A}^{-1} (t^i_j) \mathbf{A}
\end{equation}
where $A$ is the matrix $(a_j^i)$ whose elements are the ones defined in \autoref{eq:changeV}.
...
This is not surprising as both spaces are isomorphic, with a natural \emph{isomorphism} between them:
\begin{theorem}
	The map
	\begin{align*}
...
Similarly, we can study tensors of type $(2,0)$ and $(0,2)$.

For tensors of type $(2,0)$, the expression is\change{Exchange a and b roles}
\[
	t'^{kl} = \sum_{i,j}b_i^k b_j^l t^{ij},
\]
whereas for tensors of type $(0,2)$, its change of coordinates is\change{Exchange a and b roles}
\[
	t'_{kl} = \sum_{i,j} a_k^i a_l^j t_{ij}.
\]
